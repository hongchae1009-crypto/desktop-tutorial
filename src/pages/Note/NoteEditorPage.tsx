...
import type { ResearchNote, ProcedureStep, NoteStatus } from '@/types/note';
...
const SECTIONS = [
  ...
  { key: 'materials',  label: '사용 시약 및 재료', icon: '🧪' },
...
export default function NoteEditorPage({ note, onBack, onSave }: Props) {
  ...
  // 절차 단계 관리
  function addStep() {
    const step: ProcedureStep = { id: `s${Date.now()}`, text: '' };
    update('procedure', [...draft.procedure, step]);
  }
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px', background: 'var(--bg3, #f0f1f4)', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: 'var(--bg)', borderRadius: '12px', border: '1px solid var(--bd)', padding: '20px 24px' }}>
            <input
              ...
              style={{
                width: '100%', fontSize: '20px', fontWeight: 700, color: 'var(--tx)',
          ...
          <Section id="materials" icon="🧪" label="사용 시약 및 재료" onVisible={() => setActiveSection('materials')}>
            <TextArea
              value={draft.materials}
              onChange={(v) => update('materials', v)}
              placeholder="사용된 시약, 재료, 장비를 작성하세요. (한 줄에 하나씩)"
              rows={6}
              mono
            />
          </Section>
