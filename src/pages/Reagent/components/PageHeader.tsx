...
export default function PageHeader({ searchValue, onSearch, onRegister }: PageHeaderProps) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingBottom: '10px' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '5px' }}>
          {(['라벨', 'A4 인쇄', '엑셀 다운로드', '구조 검색'] as const).map((label) => (
            <button key={label} className="btn" onClick={() => showToast(`${label} 준비 중...`)}>
              {label}
            </button>
          ))}
        </div>
